...
export class UsersRepository {
  private store = new JsonStore<UserRecord>('users.json', SEED);

  findAll(): UserRecord[] { return this.store.readAll(); }
  findById(id: string): UserRecord | undefined { return this.store.readAll().find(u => u.id === id); }
  findByEmail(email: string): UserRecord | undefined { return this.store.readAll().find(u => u.email === email); }
  nextId(role: string): string { return this.store.getNextId(rolePrefix(role)); }
