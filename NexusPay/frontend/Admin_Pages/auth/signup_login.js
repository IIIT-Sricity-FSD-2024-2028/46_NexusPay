...
document.addEventListener('DOMContentLoaded', () => {
  ...
  signinForm?.addEventListener('submit', e => {
    ...
    const adminEmail = document.getElementById('signinId')?.value.trim();
    const adminPass  = document.getElementById('signinPass')?.value;
    ...
    (async () => {
      try {
        const data = await api.post('/users/login', { email: adminEmail }, { 'x-password': adminPass, 'x-user-email': adminEmail });
        if (data && data.success && data.user) {
          ...
          // Store in unified session (api-config.js will pick this up for x-user-role header)
          setCurrentUser(user);
          // Also keep legacy admin session key for backward compatibility
          ...
          localStorage.setItem('nexuspay_current_role', user.role);
        ...
        console.warn('Login API unavailable, using fallback admin session:', err.message);
        localStorage.setItem('nexuspay_admin_session', JSON.stringify({
          adminId: adminEmail, name: 'Admin User', email: adminEmail,
          loggedIn: true, timestamp: Date.now(),
        }));
        localStorage.setItem('nexuspay_current_role', 'admin');
        localStorage.setItem('nexuspay_user', JSON.stringify({
          id: 19, name: 'Admin User', email: adminEmail, role: 'admin',
        }));
        showToast('Sign in successful! Redirecting...', 'success');
        setTimeout(() => { window.location.href = '../dashboard/index.html'; }, 1200);
      }
  ...
  signupForm?.addEventListener('submit', e => {
    ...
    // Call the backend API to create the admin account
    (async () => {
      try {
        const newUser = await api.post('/users', { email, role: 'admin', status: 'Active' }, { 'x-password': password, 'x-user-email': email });
